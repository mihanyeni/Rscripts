...
ui <- fluidPage(
  ...
  conditionalPanel(
    ...
    wellPanel(
      ...
      uiOutput(outputId="selectionScaleDropOptions1")  #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Working on this to hide 'specifics' for 'Single Pop CFA' <<<<<<<<<<<<<<<<<
    ),
    wellPanel(
...
server <- function(input, output, session){
  ...
  observeEvent(c(input$selectCountry, input$selectLevel), {
    ...
    if(is.null(selectPopCurrent())){
      ...
      selectedPops <- "None"
      # If the only selection was "None", select "None"
    } else if(selectPopCurrent()=="None"){
      selectedPops <- "None"
    } else {
      ...
      if(length(selectedPops)==0){
        # If all user selected populations are deleted, select ""
        selectedPops <- ""
      }
    }
  ...
  output$warningGlobalDropLevel <- renderUI({
    if(is.null(dfCntry())){
      ...
      tags$h5("Nothing to report: (No levels in the study)")
    } else {
  ...
  output$warningGlobalDropPop <- renderUI({
    if(is.null(dfCntry())){
      ...
      tags$h5("Nothing to report: (No populations in the study)")
    } else {
  ...
  scaleOptionsList1 <- eventReactive(input$submitScaleDropOptionList, {
    if(input$checkboxesScaleDropOption[1]=="None"){
      return(NULL)
    } else if(is.na(dfCntry()$Level[1])){
      if(input$selectAnalysis=="Single Population CFA"){
        scaleOptionsInput1 <- lapply(input$checkboxesScaleDropOption, function(scl){
          ...
          tags$div(tags$h3(paste0("Options for ",scl)),
                   ...
                   # Dropping items for specific populations #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< This should only appear for the 'Single Population CFA' option
  ...
  output$selectionScaleDropOptions1 <- renderUI({
    if(is.null(dfScale())){
      return(NULL)
    } else if(input$checkboxesScaleDropOption[1]=="None"){
      tags$div(
  ...
  output$warningFunScalePI <- renderUI({
    ...
    if(warn!=0){
      tags$div(tags$h5("Ruh-roh!"), tags$img(src="RuhRoh.jpg", height="100%", width="100%"))
    } else {tags$div(tags$h5("Relax :o)"), tags$img(src="Relax.jpeg", height="100%", width="100%"))}
  })
  ...
  textWarningScaleDropItem <- eventReactive(input$submitScaleDroppingOptions, {
    # Produce text if no scales are selected
    if(input$checkboxesScaleDropOption[1]=="None"){
      tags$h5("Nothing to report: (No scales for additional dropping options selected)")
    } else {
  ...
  textFromInputs <- eventReactive(input$submitAnalysisOptions, {
    ...
    #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< reorganize, and change only minimally to understand the form of outputs <<
    ## Study name
    ...
    if(input$selectAnalysis=="Cross-country MGCFA with MI testing" | input$selectAnalysis=="Cross-country, cross-level MGCFA"){
      selectionFS              <- "No"
    } else {
